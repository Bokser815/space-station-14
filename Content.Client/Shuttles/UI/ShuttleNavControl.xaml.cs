...
public sealed partial class ShuttleNavControl : BaseShuttleControl
{
    ...
    protected override void Draw(DrawingHandleScreen handle)
    {
        ...
        var query = EntManager.EntityQuery<ActiveJetpackComponent>();

        foreach (var component in query)
        {
            var uidnew = component.Owner;  // Accessing the EntityUid from the component
            

            // Get the player's position in the world
            var playerPos = _transform.GetWorldPosition(uidnew);

            if (EntManager.TryGetComponent(component.Owner, out TransformComponent? transComp))
            {
                var pos = _transform.GetMapCoordinates(component.Owner, xform: transComp);
                Logger.Debug($"advanced position: {pos}");
            }
            

            // Log transformed position for debugging
            var finalpos = Vector2.Transform(playerPos, worldToShuttle * shuttleToView);
            
            // Define the rectangle vertices relative to the transformed position
            if (finalpos.X != 0 && finalpos.Y != 0)
            {
                handle.DrawCircle(finalpos, 5, Color.ToSrgb(Color.Red), true);
            }
